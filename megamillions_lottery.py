#!/usr/bin/env python3
"""
Mega Millions data updater and dynamic number suggester.
...
EXCEL_FILE = "megamillions_game.xlsx"

# NY State Gaming Commission – Mega Millions Winning Numbers: Beginning 2002
NY_API_URL = (
...
def fetch_latest_megamillions() -> list[dict]:
    """Fetch the 100 most recent Mega Millions draws from NY Open Data."""
    resp = requests.get(NY_API_URL, timeout=10)
    resp.raise_for_status()
    rows = []
    for item in resp.json():
        date = item.get("draw_date", "")[:10]          # "YYYY-MM-DD"
        winning = item.get("winning_numbers", "")       # "n1 n2 n3 n4 n5"
        mega_ball = item.get("mega_ball", "")
        parts = winning.split()
        if len(parts) < 5 or not mega_ball:
            continue
...
def _init_excel(filename: str) -> pd.DataFrame:
    """Create a blank Excel file with the correct columns if it doesn't exist."""
    cols = ["GameName", "Month", "Day", "Year",
            "Num1", "Num2", "Num3", "Num4", "Num5", "MegaBall"]
    df = pd.DataFrame(columns=cols)
    df.to_excel(filename, index=False)
    return df


def update_excel(filename: str = EXCEL_FILE) -> pd.DataFrame:
    """Append any draws not yet in the Excel file."""
    try:
        df = pd.read_excel(filename)
    except FileNotFoundError:
        print(f"'{filename}' not found — creating it now.")
        df = _init_excel(filename)

    new_rows = fetch_latest_megamillions()
    added = 0
    for row in new_rows:
        mask = (
            (df["Year"]     == row["Year"])
...
def suggest_numbers(df: pd.DataFrame, strategy: str = "hot") -> tuple[list[int], int]:
    ...
    strategy:
      'hot'   – top 5 most-frequent main numbers + hottest Mega Ball
    ...
    mb_freq = Counter(int(v) for v in recent["MegaBall"].dropna())

    hot_main = [n for n, _ in main_freq.most_common(10)]
    hot_mb   = mb_freq.most_common(1)[0][0]
        ...
        while len(fresh) < 5:
            for n in hot_main:
                ...
                if len(fresh) >= 5:
                    break
        main = sorted(fresh[:5])
        mb   = mb_freq.most_common(2)[1][0] if len(mb_freq) >= 2 else hot_mb

    return main, mb


def display_suggestion(label: str, main: list[int], mb: int) -> None:
    print(f"\n{'='*52}")
    print(f"  {label}")
    print(f"{'='*52}")
    print(f"  Main Numbers : {' - '.join(map(str, main))}")
    print(f"  Mega Ball    : {mb}")
    print(f"{'='*52}")


if __name__ == "__main__":
    print("Fetching latest Mega Millions results…")
    try:
    ...
    print(
        ...
        f"Mega Ball: {int(last['MegaBall'])}"
    )

    print("\n=== DYNAMIC SUGGESTIONS FOR TODAY ===")
    hot_main, hot_mb     = suggest_numbers(df, strategy="hot")
    fresh_main, fresh_mb = suggest_numbers(df, strategy="fresh")

    display_suggestion("Strategy 1 – HOT (most frequent since 2024)", hot_main, hot_mb)
    display_suggestion("Strategy 2 – HOT + FRESH (skip last 3 draws)", fresh_main, fresh_mb)
